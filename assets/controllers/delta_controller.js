...
class RangeController extends Controller {
  ...
  connect() {
    this.updateProgress();
    window.addEventListener('live:form:reset', () =>  this.updateProgress(true));
    window.addEventListener('live:form:save', () =>  this.updateProgress());
  }

  disconnect() {
    window.removeEventListener('live:form:reset', () => this.updateProgress(true));
    window.removeEventListener('live:form:save', () =>  this.updateProgress());
  ...
  updateProgress(reset = false) {
    const minValue = parseInt(reset ? this.minTarget.min : this.minTarget.value);
    const maxValue = parseInt(reset ? this.maxTarget.max : this.maxTarget.value);
